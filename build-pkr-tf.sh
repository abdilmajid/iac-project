...
CHK_PKR_CTRL=$(jq -r '.builds[-1].artifact_id' images/ami-control/manifest.json | cut -d ":" -f2)
CHK_PKR_CTRL=$(echo $?)

# Check if ami-managed exists
CHK_PKR_MNG=$(jq -r '.builds[-1].artifact_id' images/ami-managed/manifest.json | cut -d ":" -f2)
CHK_PKR_MNG=$(echo $?)
